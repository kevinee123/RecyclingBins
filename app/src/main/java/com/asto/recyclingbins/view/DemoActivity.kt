package com.asto.recyclingbins.view
...
class DemoActivity : BaseActivity() {
    ...
    override fun initData() {
        ...
        adapter = MainAdapter(listOf("垃圾门", "投递门", "投递门面板", "灯", "风扇", "称重", "跳转显示设置界面", "语音播放测试", "测试定时器", "拍照"))
    ...
    override fun bindinOnClickListener() {
        mOnSerialPortDataListener = object : OnSerialPortDataListener {

            override fun onSuccess(commandType: String, data: String) {
                when (commandType) {
                    //称重
                    Common.SERIAL_PORT_TYPE_WEIGH -> {
                        val defaultDf = DecimalFormat("#0.00")
                        var weigh = Integer.parseInt(data, 16)
                        mResultTv.text = defaultDf.format(weigh.toDouble() / 100) + "KG"
                    }
                }
            }
        }
